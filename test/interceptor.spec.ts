...
describe('interceptors', () => {
  ...
  test('should add a request interceptors', () => {
  ...
  test('should add a request interceptor that return a new config onject', () => {
    const instance = axios.create()
  ...
  test('should add a response interceptor that returns a promise', done => {
    ...
    instance.interceptors.response.use(data => {
      return new Promise(resolve => {
        //do something async
        setTimeout(() => {
          data.data = 'you hava been promised!'
          resolve(data)
        }, 10)
      })
    })
    ...
    getAjaxRequest().then(request => {
      ...
      setTimeout(() => {
        expect(response.data).toBe('you hava been promised!')
        done()
      }, 100)
    })
  })
  ...
  test('should allow removing interceptors', done => {
    ...
    insatnce.interceptors.response.use(data => {
      data.data = data.data + '1'
      return data
    })
    intercept = insatnce.interceptors.response.use(data => {
      data.data = data.data + '2'
      return data
    })
    insatnce.interceptors.response.use(data => {
    ...
    insatnce.interceptors.response.eject(intercept)
    insatnce.interceptors.response.eject(5)

    insatnce('/foo').then(data => {
      response = data
    })
